...
int LIMIT_TIME= 500, LIMIT_MEMORY = 10240;
float MAX_TIME = 100;
...
{
    printf("System time  microseconds %ld\n",usage->ru_stime.tv_usec );
    printf("User time microseconds %ld\n",usage->ru_utime.tv_usec );
    printf("Heap size %li\n",usage->ru_ixrss );
}
        ...
        for (int col = id * COL_PER_P; col < maxCol;) {
            for (int j = 0; j < COL_PER_P && col < maxCol; j++) {
                ...
                if (calcTime(cStart, cEnd) > MAX_TIME) {
                    printf("PID %i Usage self:\n", (int) getpid());
                    getrusage(RUSAGE_SELF ,&usageM);
                    printUsage(&usageM);
                    exit(exitCode);
                }
...
int main(int argc, char **argv) {
    ...
    if (argc >= 7) {
        ...
        if (sscanf(argv[++argument], "%i", &LIMIT_TIME) != 1)
            showError("Not int");
        if (LIMIT_TIME < 0)
            showError("Negative number");

        if (sscanf(argv[++argument], "%i", &LIMIT_MEMORY) != 1)
            showError("Not int");
        if (LIMIT_TIME < 0)
            showError("Negative number");
    }
...
//        showError("Matrix elooo");
    ...
    for (int k = 0; k < PROCESS_COUNT; ++k) {
        ...
        if (childPid[k] == 0) {
            ...
            rlim.rlim_max=LIMIT_TIME;
            setrlimit(RLIMIT_CPU,&rlim);
            struct rlimit rlim2;
            getrlimit(RLIMIT_MEMLOCK, &rlim2);
            rlim2.rlim_max=LIMIT_MEMORY;
            setrlimit(RLIMIT_MEMLOCK,&rlim2);
...
//          wait(&childPid[k]);
//        if (WIFEXITED(childPid[k]))
//            printf("Proces %i wykonał %i mnożeń macierzy \n", childPid[k], WEXITSTATUS(childPid[k]));
//        else if (WIFSIGNALED(childPid[k]))
//            psignal(WTERMSIG(childPid[k]), "Exit signal");
    }
    ...
    getrusage(RUSAGE_SELF ,&usageM);
    printUsage(&usageM);
    printf("Usage childrens:\n");
    getrusage(RUSAGE_CHILDREN ,&usageM);
    printUsage(&usageM);
